#!/usr/bin/env python3
"""
Base extraction strategy classes for the cry_a_4mcp.crawl4ai package.

This module provides the base classes for different extraction strategies,
...
class ExtractionStrategy(ABC):
    ...
    This class defines the common interface that all extraction strategies must implement.
    """
...
class LLMExtractionStrategy(ExtractionStrategy):
    ...
    @measure_performance
    async def extract(self, url: str, content: str, instruction: Optional[str] = None, 
                     schema: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Extract information from content using an LLM.
        
        Args:
            url: The URL of the content
            content: The HTML or markdown content to extract information from
        ...
        if len(content) > content_limit:
            logger.info(f"Content truncated from {len(content)} to {content_limit} characters")
